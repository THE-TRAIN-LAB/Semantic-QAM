# 📡 Semantic QAM: Importance-Aware Constellation Design

[![Framework: TensorFlow](https://img.shields.io/badge/Framework-TensorFlow_2.13+-FF6F00?logo=tensorflow)](#)
[![Library: NVIDIA Sionna](https://img.shields.io/badge/Library-NVIDIA_Sionna-76B900?logo=nvidia)](#)
[![Research: TRAIN LAB](https://img.shields.io/badge/Research-TRAIN_LAB-005596)](#)
...
<img src="SEM_QAM_ANIM.gif" width="600" alt="Semantic QAM Animation" />
...
## ✨ Key Features
- **Dynamic Rate Control:** Uses an RL agent to adjust the bits-per-concept based on instantaneous channel conditions.
...
## 📁 File Description
- `Semantic_QAM_21_April_2026.ipynb`: The main Jupyter Notebook containing the full end-to-end pipeline. This includes data generation (e.g., FSDD audio to spectrogram processing), VQ-VAE training, RL agent setup, and final evaluation over the SIONNA-simulated channel.
- 
...
2. If using the FSDD audio dataset, fetch the external data by cloning it to your local directory: 
   ```bash
   git clone [https://github.com/Jakobovski/free-spoken-digit-dataset.git](https://github.com/Jakobovski/free-spoken-digit-dataset.git)
   ```
...
| :--- | :--- |
| `DATASET_NAME` | The master toggle for selecting the dataset. Options include: `'fashion_mnist'`, `'mnist'`, and `'fsdd'` (audio). |
...
* `tensorflow >= 2.13`
* `sionna-no-rt == 1.2.1`
* `librosa` *(required for audio datasets)*
